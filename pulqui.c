...
static void pq_wavtoarray16(t_pulqui *x)
{
    ...
    for (i=0; i < (x->sampleperchannel); i++)
    {
       ...
       if (x->channels == 2) x->ramch2[i] = pq_positive_16(x);
    }
}
...
static void pq_wavtoarray24(t_pulqui *x)
{
    ...
    for (i=0; i < (x->sampleperchannel); i++)
    {
       ...
       if (x->channels == 2) x->ramch2[i] = pq_positive_24(x);
    }
}
...
static void pq_wavtoarray32(t_pulqui *x)
{
    ...
    for (i=0; i < (x->sampleperchannel); i++)
    {
        ...
        if (x->channels == 2) x->ramch2[i] = pq_positive_32(x);
    }
}
...
{
    pq_bee32(x, x->ramch1);
    if (x->channels == 2) pq_bee32(x, x->ramch2);
}
...
void pulqui_setup(void)
{
    logpost(NULL,2," ");
    logpost(NULL,2,"***** pulqui *****");
    logpost(NULL,2,"loaded version 0.1.2");
    logpost(NULL,2,"******************");
